...
namespace profile_service::managers
{
    S3Manager::S3Manager(const userver::components::ComponentConfig& config,
        ...
        httpClient_(context.FindComponent<userver::components::HttpClient>().GetHttpClient()),
    ...
    {
        auto connectionConfig = userver::s3api::ConnectionCfg(std::chrono::milliseconds{1000}, 2, std::nullopt);

        auto s3Connection = userver::s3api::MakeS3Connection(
            httpClient_, userver::s3api::S3ConnectionType::kHttps, "storage.yandexcloud.net", connectionConfig);

        auto auth = std::make_shared<userver::s3api::authenticators::AccessKey>(accessKey_, userver::s3api::Secret(secretKey_));
