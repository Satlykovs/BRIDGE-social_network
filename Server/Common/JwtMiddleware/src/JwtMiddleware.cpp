...
{
    JwtMiddleware::JwtMiddleware(std::string secretKey) : secretKey_(std::move(secretKey)) {};

    void JwtMiddleware::HandleRequest(userver::server::http::HttpRequest& request,
                                      userver::server::request::RequestContext& context) const
    {
        ...
        {
            throw userver::server::handlers::Unauthorized(userver::server::handlers::ExternalBody{"Invalid token format"});
        }
        ...
        {
            throw userver::server::handlers::Unauthorized(userver::server::handlers::ExternalBody{"Invalid token"});
        }
    ...
    std::string JwtMiddleware::ExtractUserId(const std::string& token) const
    {
        try
        {
            ...
            if (auto subject = decoded.get_subject(); !subject.empty()) return subject;
    ...
    ) : HttpMiddlewareFactoryBase(config, context), secretKey_(config["secret-key"].As<std::string>()) {}
