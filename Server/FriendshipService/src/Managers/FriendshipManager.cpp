...
namespace friendship_service
{
    ...
    FriendshipManager::FriendshipManager(const userver::components::ComponentConfig& config, const userver::components::ComponentContext& context) :
    userver::components::ComponentBase(config, context),
    FriendshipRepository_(
        context.FindComponent<FriendshipRepository>()
    ) {}
    ...
    userver::formats::json::Value FriendshipManager::TryGetFriendshipStatus(const friendship_service::models::FriendToActionDTO& user_data){
        ...
        if (status == "accepted"){
            builder["friendship_status"] = "Added to friends. It is possible to remove friend.";
        }
            ...
            throw std::runtime_error(fmt::format("Unexpected friendship status: {}", status));
        }
    ...
    userver::formats::json::Value FriendshipManager::TryRemoveFriend (const friendship_service::models::FriendToActionDTO& user_data){
        ...
        if (removeFriendsCount == 1 || removeFriendsCount == 0){
            ...
            throw std::runtime_error(fmt::format("Unexpected number of removed friends: {}", removeFriendsCount));
        }
    ...
    userver::formats::json::Value FriendshipManager::TryRevokeFriendshipRequest(const friendship_service::models::FriendToActionDTO& user_data){
        ...
        if (revokeFriendshipRequestsCount == 1 || revokeFriendshipRequestsCount == 0){
            builder["revoke_friendship_requests_count"] = revokeFriendshipRequestsCount;
        }
        else{
            throw std::runtime_error(fmt::format("Unexpected number of revoked friendship requests: {}", revokeFriendshipRequestsCount));
        }
    ...
    userver::formats::json::Value FriendshipManager::TrySendFriendshipRequest(const friendship_service::models::FriendToActionDTO& user_data){
        ...
        if (currentFriendshipRelationStatus.IsEmpty()){
            userver::storages::postgres::ResultSet result = FriendshipRepository_.SendFriendshipRequestQuery(currentUserID, receiverID);
    ...
    userver::formats::json::Value FriendshipManager::TryUpdateFriendRequestStatus(const friendship_service::models::DecisionFriendRequestDTO& user_data){
        ...
        userver::storages::postgres::ResultSet result = FriendshipRepository_.UpdateFriendRequestStatusQuery(currentUserID, senderID, decision);
        ...
        default:
            {
                throw std::runtime_error(fmt::format("Query affected {} rows (expected 1)", FriendshipRequestsUpdated));
            }
        }
    }
