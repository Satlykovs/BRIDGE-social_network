...
namespace chat_service::http_handlers
{
	...
	namespace
	{
		const userver::storages::postgres::Query
			kCheckActiveParticipantForLeave{
		...
		userver::server::request::RequestContext& request_context) const
	{
		...
		try
		{
			auto update_result = pg_cluster_->Execute(
				...
				kSetCurrentUserLeftAt, conversation_id_val,
				current_user_id_val);

			if (update_result.RowsAffected() > 0)
			{
				LOG_INFO() << "User " << current_user_id_val
						   << " left conversation " << conversation_id_val;
			}
			else
			{
				LOG_INFO() << "User " << current_user_id_val
		...
		{
			LOG_ERROR() << "Generic error while leaving conversation: "
						<< e.what();
			trx.Rollback();
