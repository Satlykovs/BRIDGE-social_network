...
const userver::storages::postgres::Query kCheckChatAdminForAdd{
    ...
    "NULL",
    userver::storages::postgres::Query::Name{"check_chat_admin_for_add_v3"},
};

const userver::storages::postgres::Query kCheckTargetParticipantStatusForAdd{
    "SELECT role, left_at FROM participants WHERE conversation_id = $1 AND "
    "user_id = $2",
...
const userver::storages::postgres::Query kUpsertParticipant{
    "INSERT INTO participants (conversation_id, user_id, role, left_at, "
    "last_read_message_id, notifications_enabled) "

    "VALUES ($1, $2, $3, NULL, NULL, TRUE) "
    "ON CONFLICT (conversation_id, user_id) DO UPDATE SET "
    "role = EXCLUDED.role, "
    "joined_at = CASE WHEN participants.left_at IS NOT NULL THEN NOW() ELSE "
...
AddParticipantHandler::CheckConversationAndPermissions(
  ...
  auto target_status_result = pg_cluster_->Execute(
      userver::storages::postgres::ClusterHostType::kSlave,
      kCheckTargetParticipantStatusForAdd, conversation_id, target_user_id);
...
AddParticipantHandler::HandleRequestJsonThrow(
  ...
  try {
    ...
    throw userver::server::handlers::ClientError(
        userver::server::handlers::ExternalBody{"User not authenticated"});
  }
  ...
  ConversationAccessDetails access_details = CheckConversationAndPermissions(
  ...
  try {
    ...
    LOG_INFO() << "User " << current_user_id_val
               << (access_details.target_user_was_participant ? " re-added "
                                                              : " added ")
               << "user " << target_user_id_val << " to conversation "
               << conversation_id_val << " with role " << target_role_str;

  } catch (const std::exception &e) {
    LOG_ERROR() << "Generic error during participant addition: " << e.what();
    trx.Rollback();
