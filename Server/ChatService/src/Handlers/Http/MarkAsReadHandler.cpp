...
const userver::storages::postgres::Query kUpdateLastReadMessage{
    ...
    userver::storages::postgres::Query::Name{"update_last_read_message_v2"},
};
...
MarkAsReadHandler::ResponseType MarkAsReadHandler::HandleRequestJsonThrow(
  ...
  if (!IsValidMessageInConversation(last_read_message_id_val,
    ...
    LOG_INFO() << "Attempt to mark non-existent or invalid message_id "
               << last_read_message_id_val << " as read in conversation "
               << conversation_id_val;
    throw userver::server::handlers::ClientError(
        userver::server::handlers::ExternalBody{
  ...
  try {
    auto result = pg_cluster_->Execute(
        ...
        kUpdateLastReadMessage, last_read_message_id_val, conversation_id_val,
        current_user_id_val);

    if (result.RowsAffected() > 0) {
      LOG_INFO() << "User " << current_user_id_val << " marked conversation "
                 << conversation_id_val << " as read up to message "
                 << last_read_message_id_val;
    } else {
      LOG_INFO() << "User " << current_user_id_val
                 << " tried to mark conversation " << conversation_id_val
                 << " as read up to message " << last_read_message_id_val
                 << ", but no update was needed.";
    }
    trx.Commit();

  } catch (const std::exception &e) {
    LOG_ERROR() << "Generic error while marking message as read: " << e.what();
    trx.Rollback();
