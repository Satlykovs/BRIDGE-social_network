...
namespace chat_service::http_handlers
{
	...
	namespace
	{
		const userver::storages::postgres::Query
			kGetConversationBaseInfoDetails{
				"SELECT id, type, title, last_message_id, created_at, "
				"updated_at "
				"FROM conversations "
		...
		const userver::storages::postgres::Query
			kCheckIsActiveParticipantForDetails{
		...
		int conversation_id, int64_t current_user_id) const
	{
		auto result = pg_cluster_->Execute(
			...
			kCheckIsActiveParticipantForDetails, conversation_id,
			current_user_id);
			...
			for (const auto& participant_row : participants_res)
			{
				...
				if (!participant_row["notifications_enabled"].IsNull())
				{
					p_builder["notifications_enabled"] =
						participant_row["notifications_enabled"]
