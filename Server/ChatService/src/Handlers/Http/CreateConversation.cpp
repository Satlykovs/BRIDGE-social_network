...
namespace chat_service::http_handlers
{
		...
		userver::server::request::RequestContext& request_context) const
	{
		...
		{
			auto conv_result =
				trx.Execute(kInsertConversation, type_str, title_opt);
			...
			trx.Execute(kInsertParticipant, new_conv_id_val,
						current_user_id_val, creator_role_str);

			for (const auto& participant_id_val : participant_user_ids)
			{
				trx.Execute(kInsertParticipant, new_conv_id_val,
							participant_id_val, kParticipantRoleMember);
			}
		...
		{
			LOG_ERROR() << "Generic error during conversation creation: "
						<< e.what();
			trx.Rollback();
