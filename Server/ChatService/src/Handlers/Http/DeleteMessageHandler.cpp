...
namespace chat_service::http_handlers
{
	...
	namespace
	{
		...
		const userver::storages::postgres::Query kSoftDeleteMessage{
			"UPDATE messages SET deleted = TRUE, deletion_reason = 'user', "
			"edited_at = "
			"NOW() "
	...
	DeleteMessageHandler::ResponseType
	DeleteMessageHandler::HandleRequestJsonThrow(
		...
		userver::server::request::RequestContext& request_context) const
	{
		...
		MessageDetailsForDelete message_details =
			GetMessageDetailsAndCheckPermissions(message_id_val,
												 current_user_id_val);
		...
		{
			LOG_INFO() << "Attempt to delete already deleted message "
					   << message_id_val << " by user " << current_user_id_val;
			http_request.SetResponseStatus(
		...
		{
			auto delete_result =
				trx.Execute(kSoftDeleteMessage, message_id_val);

			if (delete_result.RowsAffected() == 0)
			{
				LOG_WARNING()
					<< "Message " << message_id_val
			...
			LOG_INFO() << "User " << current_user_id_val
					   << " soft-deleted message " << message_id_val;
		}
		catch (const std::exception& e)
		{
			LOG_ERROR() << "Generic error during message soft-deletion: "
						<< e.what();
			trx.Rollback();
