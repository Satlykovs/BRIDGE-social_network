...
const userver::storages::postgres::Query kListUserConversations{
    ...
    userver::storages::postgres::Query::Name{"list_user_conversations_v4"},
};
  ...
  try {
    ...
    for (const auto &row : pg_result) {
      ...
      if (!row["last_message_id"].IsNull()) {
        conv_builder["last_message_id"] = row["last_message_id"].As<int64_t>();
      }
    ...
    LOG_ERROR() << "Generic error while listing conversations: " << e.what();
