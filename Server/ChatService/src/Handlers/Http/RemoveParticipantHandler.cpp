...
namespace chat_service::http_handlers
{
	...
	namespace
	{
		const userver::storages::postgres::Query
			kCheckDetailsForParticipantRemoval{
				"SELECT "
		...
		int64_t user_id_to_remove) const
	{
		...
		details.conversation_type_str =
			row["conversation_type"].As<std::string>();
		...
		userver::server::request::RequestContext& request_context) const
	{
		...
		ParticipantRemovalCheckDetails check_details =
			CheckPermissionsForRemoval(conversation_id_val, current_user_id_val,
									   user_id_to_remove_val);
		...
		try
		{
			auto update_result = pg_cluster_->Execute(
				...
				kSetParticipantLeftAt, conversation_id_val,
				user_id_to_remove_val);

			if (update_result.RowsAffected() > 0)
			{
				LOG_INFO() << "Admin " << current_user_id_val
						   << " removed participant " << user_id_to_remove_val
						   << " from conversation " << conversation_id_val;
			}
			else
			{
				LOG_INFO() << "User " << user_id_to_remove_val
						   << " was not an active participant or already left "
							  "conversation "
						   << conversation_id_val << " when admin "
						   << current_user_id_val << " tried to remove them.";
			}
		...
		{
			LOG_ERROR() << "Generic error during participant removal: "
						<< e.what();
			trx.Rollback();
