...
const std::string kListMessagesQueryBase =
    "SELECT id, conversation_id, sender_id, content, attachments, reply_to_id, "
    "sent_at, edited_at "
    "FROM messages "
    "WHERE conversation_id = $1 AND deleted = FALSE ";

const userver::storages::postgres::Query kCheckParticipantForGetMessages{
...
GetMessagesHandler::ResponseType GetMessagesHandler::HandleRequestJsonThrow(
  ...
  if (http_request.HasArg("limit")) {
    try {
      ...
      LOG_WARNING() << "Failed to parse 'limit' query_param: " << e.what()
                    << ". Using default 50.";
    }
  }
  ...
  if (http_request.HasArg("before_message_id")) {
    try {
      ...
      LOG_WARNING() << "Failed to parse 'before_message_id' query_param: "
                    << e.what();
      throw userver::server::handlers::ClientError(
          userver::server::handlers::ExternalBody{
  ...
  if (before_message_id_val) {
    query_string += "AND id < $" + std::to_string(params.Size() + 1) + " ";
  ...
  try {
    ...
    for (const auto &row : pg_result) {
      ...
      msg_builder["conversation_id"] = row["conversation_id"].As<int>();
      ...
      if (!row["reply_to_id"].IsNull()) {
        msg_builder["reply_to_id"] = row["reply_to_id"].As<int64_t>();
      }
      ...
      if (!row["edited_at"].IsNull()) {
        msg_builder["edited_at"] = userver::utils::datetime::Timestring(
            row["edited_at"].As<std::chrono::system_clock::time_point>());
      }
