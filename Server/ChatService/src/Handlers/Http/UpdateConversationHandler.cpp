...
namespace chat_service::http_handlers
{
	...
	namespace
	{
		const userver::storages::postgres::Query
			kCheckConversationAdminForUpdate{
		...
		const userver::storages::postgres::Query kUpdateConversationDetails{
			...
			"WHERE id = $2 "
			"RETURNING id, type, title, last_message_id, created_at, "
			"updated_at",
			userver::storages::postgres::Query::Name{
				"update_conversation_details_v4"},
		};
		...
		userver::server::request::RequestContext& request_context) const
	{
		...
		try
		{
			...
			LOG_INFO() << "Admin " << current_user_id_val
					   << " updated title of conversation "
					   << conversation_id_val << " to '" << title_str << "'";
		}
		catch (const std::exception& e)
		{
			LOG_ERROR() << "Generic error during conversation update: "
						<< e.what();
			trx.Rollback();
