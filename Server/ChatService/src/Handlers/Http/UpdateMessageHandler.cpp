...
const userver::storages::postgres::Query kUpdateMessageContent{
    ...
    userver::storages::postgres::Query::Name{"update_message_content_v3"},
};

const userver::storages::postgres::Query kUpdateConversationTimestampOnEdit{
...
UpdateMessageHandler::ResponseType UpdateMessageHandler::HandleRequestJsonThrow(
  ...
  try {
    ...
    if (update_result.IsEmpty()) {
      throw std::runtime_error("DB error: Message update failed unexpectedly.");
    }
    ...
    response_message_builder["sender_id"] = row["sender_id"].As<int64_t>();
    if (!row["content"].IsNull()) {
      response_message_builder["content"] = row["content"].As<std::string>();
    }
