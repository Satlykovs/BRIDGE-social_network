...
namespace auth_service::managers
{
	...
	JwtManager::JwtManager(const userver::components::ComponentConfig& config,
		...
		: ComponentBase(config, context),
		  ...
		  jwtRepository_(
			  context
				  .FindComponent<auth_service::repositories::JwtRepository>())
	{
		LOG_INFO() << "JwtManager initialized with token lifetime: "
				   << accessTokenLifetime_.count() << " minutes";
	}

	std::string JwtManager::CreateToken(
	...
	JwtManager::GenerateAccessToken(int userId) const
	{
		auto expTime = std::chrono::system_clock::now() + accessTokenLifetime_;
		return {CreateToken(userId, expTime),
				userver::utils::datetime::TimePointTz(expTime)};
	}

	std::pair<std::string, userver::utils::datetime::TimePointTz>
	JwtManager::GenerateRefreshToken(int userId) const
	{
		auto expTime = std::chrono::system_clock::now() + refreshTokenLifetime_;
		std::string token = CreateToken(userId, expTime);

		jwtRepository_.AddRefreshToken(
			userId, token,
			userver::storages::postgres::TimePointWithoutTz(expTime));
